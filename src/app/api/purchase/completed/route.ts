...
import { createInterval } from "@/lib/utils";
...
export async function POST(request: NextRequest) {
    ...
    if (hash === signature) {
        ...
        let durationMinutes: number | null = -1;
...
export async function GET(request: NextRequest) {
    ...
    if (isDev ? false : !request_ip) {
        return NextResponse.json({
            status: 400,
            error: "bad request (ip not found), please contact support (https://www.mspaint.cc/support)"
        })
    }
    ...
    if (!order_id || !order_email) {
        return NextResponse.json({
            status: 400,
            error: "bad request, please contact support (https://www.mspaint.cc/support)"
        })
    }
    ...
    if (!response.ok) {
        return NextResponse.json({
            status: 500,
            error: "internal server error, please contact support (https://www.mspaint.cc/support)"
        })
    }
    ...
    if (invoiceData.customer_information.email !== order_email || invoiceData.status.status.status !== "COMPLETED" || (!isDev && invoiceData.customer_information.ip !== request_ip)) {
        return NextResponse.json({
            status: 400,
            error: "bad request (data mismatch), please contact support (https://www.mspaint.cc/support)"
        })
    }

    await sql`CREATE TABLE IF NOT EXISTS public.mspaint_keys_new (
    ...
    return NextResponse.json({
        status: 400,
        error: "bad request (all keys claimed), please contact support (https://www.mspaint.cc/support)"
    })
}
